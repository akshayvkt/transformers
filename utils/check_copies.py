...
def is_copy_consistent(filename: str, overwrite: bool = False) -> Optional[List[Tuple[str, int]]]:
    ...
    while line_index < len(lines):
        ...
        # ignore empty blocks, and the blocks specified to be ignored
        # this is the version used to check if there is a mismatch
        theoretical_code_blocks_clean = {k: v for k, v in theoretical_code_blocks.items() if not (k.startswith(("_empty_block_", "_ignored_existing_block_", "_ignored_new_block_")))}
        theoretical_code = ''.join(list(theoretical_code_blocks_clean.values()))

        observed_code_blocks_clean = {k: v for k, v in observed_code_blocks.items() if not (k.startswith(("_empty_block_", "_ignored_existing_block_", "_ignored_new_block_")))}
